...
public class ItemShopsCreatorAdvanced {
	...
	public BSBuy createBuyItem(BSShops shophandler, BSShop shop, ISItem item, BossShop plugin, BSRewardType rewardtype, BSPriceType pricetype, double reward_multiplier, double price_multiplier, boolean worth_is_for_one_unit){
		...
		final Number buyPrice = item.getWorth(true, price_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		final Number sellPrice = item.getWorth(false, reward_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		buy.setItem(preview.getMenuItem(item.getItemData(), item.getItemStack(), 1, buyPrice, sellPrice), false);
	...
	public BSShop createSubShop(BSShops shophandler, BSShop shop, BSBuy buy, ISItem item, String shop_name, BossShop plugin, BSRewardType rewardtype, BSPriceType pricetype, double reward_multiplier, double price_multiplier, boolean worth_is_for_one_unit){
		final Number buyPrice = item.getWorth(true, price_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		final Number sellPrice = item.getWorth(false, reward_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		BSShop individual_shop = new BSShop(shophandler.createId(), shop_name, null, true, plugin, ClassManager.manager.getStringManager().transform(preview.transformEntry(look_adv_subshop_displayname.replace("%parentshopname%", shop.getValidDisplayName(null, null)), item.getItemStack(), item.getItemStack().getAmount(), buyPrice, sellPrice), buy, shop, null, null), 0, null) {
		...
		if(levels != null){
			...
			for(int level = 0; level < levels.length; level++){
				...
				if(item.allowBuy()){
					addBuyItemBuy(individual_shop, pricetype, rewardtype, item, price_multiplier, reward_multiplier, first_slot, amount, worth_is_for_one_unit);
				}else if(item.allowSell()){
					addBuyItemSell(individual_shop, rewardtype, item, price_multiplier, reward_multiplier, first_slot, amount, worth_is_for_one_unit);
				}

				if(item.allowSell()){
					addBuyItemSell(individual_shop, rewardtype, item, price_multiplier, reward_multiplier, first_slot+2, amount, worth_is_for_one_unit);
				} else if (item.allowBuy()){
					addBuyItemBuy(individual_shop, pricetype, rewardtype, item, price_multiplier, reward_multiplier, first_slot+2, amount, worth_is_for_one_unit);
				}
			}
		...
		if(item.allowBuyAll() && item.allowSellAll()){
			addBuyItemBuyAll(individual_shop, pricetype, rewardtype, item, price_multiplier, reward_multiplier, (levels.length)*9 + 3, worth_is_for_one_unit);
			addBuyItemSellAll(individual_shop, rewardtype, item, price_multiplier, reward_multiplier, (levels.length)*9 + 5, worth_is_for_one_unit);
		}else{

			if(item.allowSellAll()){
				addBuyItemSellAll(individual_shop, rewardtype, item, price_multiplier, reward_multiplier, (levels.length)*9 + 4, worth_is_for_one_unit);
			}else if(item.allowBuyAll()){
				addBuyItemBuyAll(individual_shop, pricetype, rewardtype, item, price_multiplier, reward_multiplier, (levels.length)*9 + 4, worth_is_for_one_unit);
			}
	...
	private BSBuy addBuyItemBuy(BSShop individual_shop, BSPriceType pricetype, BSRewardType rewardtype, ISItem item, double price_multiplier, double rewardMultiplier, int slot, int amount, boolean worth_is_for_one_unit){
		...
		final Number buyPrice = item.getWorth(true, price_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		final Number sellPrice = item.getWorth(false, rewardMultiplier, 1, rewardtype, true, worth_is_for_one_unit);
		shopitem.setShop(individual_shop);
		individual_shop.addShopItem(shopitem, buy.getMenuItem(null, null, amount, buyPrice, sellPrice), ClassManager.manager);
		return shopitem;
	}

	private BSBuy addBuyItemBuyAll(BSShop individual_shop, BSPriceType pricetype, BSRewardType rewardtype, ISItem item, double price_multiplier, double reward_multiplier, int slot, boolean worth_is_for_one_unit){
		...
		final Number sellPrice = item.getWorth(false, reward_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		shopitem.setShop(individual_shop);
		individual_shop.addShopItem(shopitem, buyall.getMenuItem(null, null, 1, buyPrice, sellPrice), ClassManager.manager);
		return shopitem;
	}

	private BSBuy addBuyItemSell(BSShop individual_shop, BSRewardType rewardtype, ISItem item, double price_multiplier, double reward_multiplier, int slot, int amount, boolean worth_is_for_one_unit){
		...
		shopitem.setShop(individual_shop);
		final Number buyPrice = item.getWorth(true, price_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		final Number sellPrice = item.getWorth(true, reward_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		individual_shop.addShopItem(shopitem, sell.getMenuItem(null, null, amount, buyPrice, sellPrice), ClassManager.manager);
		return shopitem;
	}

	private BSBuy addBuyItemSellAll(BSShop individual_shop, BSRewardType rewardtype, ISItem item, double price_multiplier, double reward_multiplier, int slot, boolean worth_is_for_one_unit){
		ItemStack i = item.getItemStack().clone();
		i.setAmount(1);
		...
		final Number buyPrice = item.getWorth(true, price_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		final Number sellPrice = item.getWorth(true, reward_multiplier, 1, rewardtype, true, worth_is_for_one_unit);
		individual_shop.addShopItem(shopitem, sellall.getMenuItem(null, null, 1, buyPrice, sellPrice), ClassManager.manager);
		return shopitem;
	}
	...
	private BSBuy addBuyItemPreview(BSShop individual_shop, ISItem item, int slot, int amount){
		...
		shopitem.setShop(individual_shop);
		individual_shop.addShopItem(shopitem, preview.getMenuItem(item.getItemData(), item.getItemStack(), amount, null, null), ClassManager.manager);
		return shopitem;
	}
	...
	private BSBuy addBuyItemClose(BSShop individual_shop, int slot){
		...
		shopitem.setShop(individual_shop);
		individual_shop.addShopItem(shopitem, close.getMenuItem(null, null, 1, null, null), ClassManager.manager);
		return shopitem;
	}
	...
	private BSBuy addBuyItemBack(BSShop individual_shop, BSShop main_shop, int slot){
		...
		shopitem.setShop(individual_shop);
		individual_shop.addShopItem(shopitem, back.getMenuItem(null, null, 1, null, null), ClassManager.manager);
		return shopitem;
	}
